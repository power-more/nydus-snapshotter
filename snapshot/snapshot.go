...
func (o *snapshotter) Remove(ctx context.Context, key string) error {
	...
	blobDigestString, cleanupBlob := snap.Labels[label.NydusDataLayer]
	if cleanupBlob && blobDigestString == "true" {
		blobDigestString = snap.Labels[label.CRILayerDigest]
	}
	...
	if o.syncRemove {
		...
		defer func() {
			if err == nil {
				...
				if cleanupBlob {
					if err := o.fs.CleanupBlobLayer(ctx, blobDigestString, false); err != nil {
		...
		defer func() {
			if cleanupBlob {
				if err := o.fs.CleanupBlobLayer(ctx, blobDigestString, true); err != nil {
					log.G(ctx).WithError(err).WithField("id", key).Warn("failed to remove blob")
				}
			}
		}()
